...
def fetch_all_active_feeds():
    ...
    source_slugs = list(
        Source.objects.filter(is_active=True).values_list("slug", flat=True)
    )
...
def fetch_and_parse_feed(source_slug: str):
    ...
    existing_hashes = set(
        Article.objects.filter(url_hash__in=candidate_hashes).values_list(
            "url_hash", flat=True
        )
    )
    ...
    log.status = (
        FetchLog.Status.SUCCESS if stored == len(new_items) else FetchLog.Status.PARTIAL
    )
    log.save(
        update_fields=[
            "status",
